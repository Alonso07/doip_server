...
class TestUDSEndToEnd:
    ...
    def test_udp_vehicle_discovery_to_tcp_diagnostics(self, server):
        ...
        print(f"✅ Vehicle discovered: VIN={vehicle_info['vin']}, Address=0x{vehicle_info['logical_address']:04X}")
        ...
        try:
            ...
            print(f"✅ Routing activation successful: {routing_response['response_code']:02X}")
            ...
            diag_request = self._create_diagnostic_message_request(0x0E00, 0x1000, session_request)
            ...
            diag_request = self._create_diagnostic_message_request(0x0E00, 0x1000, vin_request)
            tcp_client.send(diag_request)
            ...
            for service_id, service_name in engine_services:
                ...
                service_request = self._create_read_data_by_identifier_request(service_id)
                diag_request = self._create_diagnostic_message_request(0x0E00, 0x1000, service_request)
                tcp_client.send(diag_request)
            ...
            diag_request = self._create_diagnostic_message_request(0x0E00, 0x1000, tester_present)
            tcp_client.send(diag_request)
    ...
    def test_multi_ecu_communication(self, server):
        ...
        try:
            ...
            engine_response = self._parse_diagnostic_message_response(tcp_client.recv(1024))
            ...
            trans_response = self._parse_diagnostic_message_response(tcp_client.recv(1024))
            ...
            abs_response = self._parse_diagnostic_message_response(tcp_client.recv(1024))
    ...
    def test_error_handling_and_validation(self, server):
        ...
        try:
            ...
            invalid_request = self._create_diagnostic_message_request(
                0x9999, 0x1000,  # Invalid source address
            ...
            invalid_request = self._create_diagnostic_message_request(
                0x0E00, 0x9999,  # Invalid target address
                self._create_read_data_by_identifier_request(0xF190)
            )
            tcp_client.send(invalid_request)
            response = tcp_client.recv(1024)
    ...
    def _create_routing_activation_request(self):
        ...
        payload = struct.pack(">HHB", 0x0E00, 0x1000, 0x00)  # Client, target, response code
...
class TestUDSEndToEndIntegration:
    ...
    def _create_routing_activation_request(self):
        """Create DoIP routing activation request"""
        header = struct.pack(">BBHI", 0x02, 0xFD, 0x0005, 7)
        payload = struct.pack(">HHB", 0x0E00, 0x1000, 0x00) + struct.pack(">I", 0x00000000)
        return header + payload
