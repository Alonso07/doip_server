...
class TestDoIPConfigurationManager:
    ...
    def test_response_code_descriptions(self):
        ...
        routine_desc = config_manager.get_response_code_description(
            "routine_activation", 0x10
        )
        assert (
            "success" in routine_desc.lower()
            or routine_desc != f"Unknown response code: 0x10"
        )
