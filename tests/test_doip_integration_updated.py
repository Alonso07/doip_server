...
class TestDoIPIntegrationHierarchical:
    ...
    def test_server_startup(self, server):
        ...
        assert server.running is True
        assert server.server_socket is not None
        assert hasattr(server.config_manager, 'get_all_ecu_addresses')  # Hierarchical config manager method
    ...
    def test_uds_read_data_by_identifier_transmission_ecu(self, server):
        ...
        try:
            ...
            response = client.send_diagnostic_to_address(0x1001, bytes([0x22, 0xF1, 0x90]))
            assert True  # Test passes if we get here
    ...
    def test_uds_read_data_by_identifier_abs_ecu(self, server):
        ...
        try:
            ...
            response = client.send_diagnostic_to_address(0x1002, bytes([0x22, 0xF1, 0x90]))
            assert True  # Test passes if we get here
    ...
    def test_ecu_specific_services(self, server):
        ...
        try:
            ...
            response = client.send_diagnostic_to_address(0x1000, bytes([0x22, 0x0C, 0x01]))
            assert True  # Test passes if we get here
            
            # Test Transmission-specific service (Gear read) for Transmission ECU
            response = client.send_diagnostic_to_address(0x1001, bytes([0x22, 0x0C, 0x0A]))
            assert True  # Test passes if we get here
            
            # Test ABS-specific service (Wheel speed read) for ABS ECU
            response = client.send_diagnostic_to_address(0x1002, bytes([0x22, 0x0C, 0x0B]))
            assert True  # Test passes if we get here
