...
"""
    ...
    def _gateway_references_target_address(self, target_address: int) -> bool:
        ...
        data, _yaml = self._read_rt(self.gateway_config_path)
        gateway = (data or {}).get("gateway", {})
        for ecu_ref in gateway.get("ecus", []) or []:
            path = self._find_ecu_config_path(ecu_ref)
            if not path or not os.path.exists(path):
                continue
            try:
                with open(path, "r") as f:
                    ecu_data = yaml.safe_load(f) or {}
            except Exception as exc:
                self.logger.warning(
                    "Could not inspect ECU reference %s: %s", ecu_ref, exc
                )
                continue
            ecu = ecu_data.get("ecu", {})
