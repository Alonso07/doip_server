...
class HierarchicalConfigManager:
    ...
    def __init__(self, gateway_config_path: str = None):
        ...
        """
        self.gateway_config_path = gateway_config_path or self._find_default_gateway_config()
        self.gateway_config = {}
...
"""
    ...
    def _load_gateway_config(self):
        ...
        try:
            ...
            self.logger.info(f"Gateway configuration loaded from: {self.gateway_config_path}")
        except Exception as e:
            ...
            try:
                ...
                if ecu_path and os.path.exists(ecu_path):
                    ...
                    if target_address is not None:
                        self.ecu_configs[target_address] = ecu_config
                        self.logger.info(f"ECU configuration loaded: {ecu_file} -> 0x{target_address:04X}")
                    else:
                        self.logger.warning(f"ECU configuration missing target_address: {ecu_file}")
                else:
    ...
    def is_source_address_allowed(self, source_addr: int, target_addr: int = None) -> bool:
    ...
    def get_ecu_uds_services(self, target_address: int) -> Dict[str, Any]:
        ...
        for service_name in all_service_names:
            if service_name in self.uds_services:
                ...
                self.logger.warning(f"Service {service_name} not found in UDS services for ECU 0x{target_address:04X}")
        
        return ecu_services
    
    def get_uds_service_by_request(self, request: str, target_address: int = None) -> Optional[Dict[str, Any]]:
        ...
        if not self.ecu_configs:
            ...
            for target_addr, ecu_config in self.ecu_configs.items():
                ...
                if 'tester_addresses' not in ecu_info:
                    self.logger.error(f"ECU 0x{target_addr:04X} missing tester_addresses")
                    return False
    ...
    def get_config_summary(self) -> str:
        ...
        summary.append(f"Network: {network.get('host', 'N/A')}:{network.get('port', 'N/A')}")
        ...
        for target_addr in self.get_all_ecu_addresses():
            ...
            summary.append(f"  - 0x{target_addr:04X}: {ecu_info.get('name', 'Unknown')}")
