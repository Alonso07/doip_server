...
class DebugDoIPClient:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"Connection parameters: host={self.server_host}, target=0x{self.target_address:04X}")
            
            # Create DoIP client instance
            self.doip_client = DoIPClient(
                self.server_host,  # IP address
                self.target_address  # Logical address
            )
            
            self.logger.info("DoIP client created successfully")
            self.logger.info(f"Connected to DoIP server at {self.server_host}:{self.server_port}")
    ...
    def send_diagnostic_message(self, uds_payload: List[int], timeout: Optional[float] = None) -> Optional[bytes]:
        """
        ...
        try:
            ...
            response = self.doip_client.send_diagnostic_message(uds_payload_bytes, timeout=timeout)
    ...
    def run_test_scenario(self, scenario_name: str) -> bool:
        ...
        if not scenario:
            self.logger.error(f"Test scenario '{scenario_name}' not found in configuration")
            return False
        ...
        for step in scenario["steps"]:
            ...
            if step == "connect":
                ...
                session_type = int(self.config["uds_services"]["diagnostic_session_control"]["session_type"], 16)
                ...
                data_ids = self.config["uds_services"]["read_data_by_identifier"]["data_identifiers"]
                for data_id_str in data_ids:
                    data_id = int(data_id_str, 16)
                    uds_payload = [0x22, (data_id >> 8) & 0xFF, data_id & 0xFF]
                    if not self.send_diagnostic_message(uds_payload):
                        self.logger.warning(f"Read data by identifier failed for 0x{data_id:04X}")
                    time.sleep(0.5)
                    
            elif step == "routine_control":
                routine_id = int(self.config["uds_services"]["routine_control"]["routine_identifier"], 16)
                routine_type = int(self.config["uds_services"]["routine_control"]["routine_type"], 16)
                uds_payload = [0x31, 0x01, (routine_id >> 8) & 0xFF, routine_id & 0xFF, routine_type]
                if not self.send_diagnostic_message(uds_payload):
        ...
        self.logger.info(f"Test scenario '{scenario_name}' completed. Success: {success}")
        return success
