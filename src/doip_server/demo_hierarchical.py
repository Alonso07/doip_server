...
def demo_configuration_loading():
    ...
    for target_addr in config_manager.get_all_ecu_addresses():
        ...
        print(
            f"     Tester Addresses: {[f'0x{addr:04X}' for addr in config_manager.get_ecu_tester_addresses(target_addr)]}"
        )
    ...
    for source_addr, target_addr, description in test_addresses:
        ...
        print(f"   {description}:")
        print(
            f"     Source 0x{source_addr:04X} allowed: {'✓' if source_valid else '✗'}"
        )
...
def demo_runtime_ecu_loading():
    ...
    for target_addr in config_manager.get_all_ecu_addresses():
        ...
        print(
            f"     Common services ({len(common_services)}): {', '.join(common_services)}"
        )
        print(
            f"     Specific services ({len(specific_services)}): {', '.join(specific_services)}"
        )
