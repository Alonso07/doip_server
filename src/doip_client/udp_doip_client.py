...
class UDPDoIPClient:
    ...
    def parse_vehicle_identification_response(self, data: bytes) -> Optional[dict]:
        ...
        if (protocol_version != self.DOIP_PROTOCOL_VERSION or 
            inverse_protocol_version != self.DOIP_INVERSE_PROTOCOL_VERSION):
    ...
    def send_vehicle_identification_request(self) -> Optional[dict]:
        ...
        try:
            ...
            self.logger.info(f"Sending vehicle identification request to {self.broadcast_address}:{self.server_port}")
            ...
            if response_data:
                ...
                self.logger.info(f"Logical Address: 0x{response_data['logical_address']:04X}")
                self.logger.info(f"EID: {response_data['eid']}")
                self.logger.info(f"GID: {response_data['gid']}")
                self.logger.info(f"Further Action Required: {response_data['further_action_required']}")
                self.logger.info(f"VIN/GID Sync Status: {response_data['vin_gid_sync_status']}")
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="UDP DoIP Client for Vehicle Identification Testing")
    parser.add_argument("--port", type=int, default=13400, help="UDP port to send requests to")
    ...
    if responses:
        ...
        for i, response in enumerate(responses):
            print(f"Response {i+1}: VIN={response['vin']}, Address=0x{response['logical_address']:04X}")
    else:
